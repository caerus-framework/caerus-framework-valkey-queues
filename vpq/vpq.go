...
// requeues the item (weight +1) and drops its deadlock tracking. Honour ctx
// for shutdown and deadlines.
...
// WithValkeyName binds this queue to a valkey component Name() other than
// the default "valkey". GetDependencies reports that name.
...
// PriorityQueue is the weighted priority-queue machine in
// caerus-framework-valkey-queues. Add increments the item weight; consumers
// pop the highest-weighted item. It holds a valkey peer pointer.
...
// overlayTunables overlays non-identity fields (not queue name).
...
func (q *PriorityQueue) GetDependencies() []string {
	deps := []string{chassis.PeerName(q.valkeyName), cf_logs.ComponentName}
	if q.configSource != "" {
...
func (q *PriorityQueue) Init(ctx context.Context, fw *cf.CaerusFramework) error {
	...
	vk, err := chassis.ResolveValkey(fw, q.valkeyName)
	if err != nil {
		return fmt.Errorf("cf_vpq: %w", err)
	}
	if q.cfg.QueueName == "" {
	...
	q.valkey = vk
	if vk.Client() == nil {
		q.logger.Warn("cf_vpq: valkey Client() is nil (degraded / not connected); queue Init succeeded, Health stays not-ready until ping works",
			"queue", q.cfg.QueueName, "valkey", chassis.PeerName(q.valkeyName))
	} else {
		q.logger.Info("cf_vpq: queue ready",
			"queue", q.cfg.QueueName,
			"valkey", chassis.PeerName(q.valkeyName),
...
func (q *PriorityQueue) applyRecoverHealthFromConfig(loaded PQConfig) {
	...
	if loaded.Workers > 0 {
		...
		if q.valkey != nil && next != q.workers {
			q.logger.Info("cf_vpq: workers change requires process restart", "from", q.workers, "to", next)
		}
		q.workers = next
...
func (q *PriorityQueue) vkKey(parts ...string) string {
	if q.valkey != nil {
		return q.valkey.Key(parts...)
	}
	return strings.Join(parts, ":")
}

func (q *PriorityQueue) stringKey(id string) string {
	return q.vkKey("squeue", q.cfg.QueueName, id)
}

func (q *PriorityQueue) payloadKeyPrefix() string {
	return q.vkKey("squeue", q.cfg.QueueName) + ":"
}

func (q *PriorityQueue) zsetKey() string {
	return q.vkKey("zqueue", q.cfg.QueueName)
}

func (q *PriorityQueue) deadlockKey() string {
	return q.vkKey("pqdeadlocks", q.cfg.QueueName)
}

func (q *PriorityQueue) expiryKey() string {
	return q.vkKey("zexpiry", q.cfg.QueueName)
}

func (q *PriorityQueue) wakeKey() string {
	return q.vkKey("pqwake", q.cfg.QueueName)
}

func (q *PriorityQueue) channel() string {
	return q.vkKey("pqchan", q.cfg.QueueName)
}
...
func (q *PriorityQueue) Consume(ctx context.Context, handler Handler) error {
	...
	go func() {
		for {
			...
			err := cli.Receive(ctx, cli.B().Subscribe().Channel(q.channel()).Build(), func(msg valkey.PubSubMessage) {
				select {
				case wake <- struct{}{}:
				default:
				}
			...
			if err != nil && !errors.Is(err, context.Canceled) && !errors.Is(err, valkey.ErrClosing) {
				q.logger.Error("cf_vpq: pub/sub subscribe ended; retrying", "err", err)
			}
			select {
			case <-ctx.Done():
				return
