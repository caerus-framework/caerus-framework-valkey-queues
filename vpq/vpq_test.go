...
func TestComponentContract(t *testing.T) {
	...
	named := New(WithQueueName("orders"), WithValkeyName("valkey-cache"))
	nd := named.GetDependencies()
	if len(nd) < 1 || nd[0] != "valkey-cache" {
		t.Fatalf("WithValkeyName GetDependencies() = %v, want valkey-cache first", nd)
	}
...
func TestKeyHelpers(t *testing.T) {
	...
	q.valkey = cf_valkey.New(cf_valkey.WithKeyPrefix("myapp:"))
	if got := q.stringKey("id1"); got != "myapp:squeue:orders:id1" {
		t.Fatalf("peer-prefixed stringKey = %q", got)
	}
}
