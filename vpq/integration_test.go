...
func TestIntegrationGhostHandling(t *testing.T) {
	...
	rq := New(WithQueueName("ghost-r"), WithBlockDuration(time.Second))
	...
	dq := New(WithQueueName("ghost-d"), WithCacheTimeout(time.Second), WithBlockDuration(time.Second))
...
func TestIntegrationHealthThresholds(t *testing.T) {
	...
	q2 := New(WithQueueName("hinf"), WithMaxInFlight(1))
...
func TestIntegrationConsume(t *testing.T) {
	...
	q := New(
		WithQueueName("ctest"),
		WithHandler(func(ctx context.Context, item *BGetObject) error {
